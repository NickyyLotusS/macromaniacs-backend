...
Este documento descreve o banco relacional do escopo atual. Ele deve ser atualizado na mesma mudança que alterar entidades TypeORM, migrations, o contrato estrutural ou `diagrama.dbml`. A transição operacional está em [orm-transition.md](orm-transition.md) e na [ADR 0002](../adr/0002-typeorm-preserving-postgresql.md).

## Tecnologias e convenções

- PostgreSQL 15.
- TypeORM 1.1.1 é o único ORM ativo. Prisma 5.22 foi usado inicialmente e seu histórico é somente legado.
- Tabelas e colunas físicas em `snake_case`; entidades/campos TypeORM em `PascalCase`/`camelCase`.
- Chaves primárias UUID v4 geradas pela aplicação com `randomUUID()`, sem default SQL. `save(repository.create(...))` usa `@BeforeInsert`; inserts/upserts/query builders devem fornecer o UUID explicitamente.
- Instantes em `TIMESTAMPTZ`; datas civis em `DATE`.
- Valores monetários da loja são pontos inteiros, não moeda fiduciária.
- `created_at` tem default `CURRENT_TIMESTAMP` no banco; `@CreateDateColumn` e `@UpdateDateColumn` preservam a auditoria. SQL externo deve atualizar `updated_at` explicitamente; não foi criado trigger novo.
- Os 50 `CHECK`s estão na baseline SQL e também em `@Check` com seus nomes originais. Índices DESC ficam em SQL e são declarados com `synchronize: false` individualmente, sem desabilitar a comparação das entidades.
- `NUMERIC` é lido/escrito como `string` exata; não converter silenciosamente para `number`. Cálculos nutricionais exigem aritmética decimal com arredondamento explícito. `DATE` usa string `YYYY-MM-DD`; `TIMESTAMPTZ` usa `Date`.
- `DatabaseSource` mantém a representação física de `CURRENT_TIMESTAMP` na comparação do driver PostgreSQL. TypeORM está fixado e a compatibilidade é coberta pelo teste de zero drift; qualquer atualização do ORM deve repetir essa verificação.

O schema preserva exatamente 23 entidades/tabelas de domínio. Funcionalidades citadas em versões antigas da documentação — refresh tokens, papel global de usuário, metas de hidratação, sódio, tipo fixo de refeição, posts globais e quests semanais — não fazem parte do escopo atual.
...
Os quatro arquivos Prisma permanecem byte a byte em `legacy/prisma/migrations`. Nunca edite seu conteúdo/checksum nem volte a executá-los no banco existente. Não há dependência ou script Prisma ativo.

A migration TypeORM `DefinitiveBaseline1789430400000` cria diretamente o schema final em banco vazio. Em banco já criado pelo Prisma, apenas essa baseline pode ser registrada com `--fake`, depois de histórico, paridade estrutural, backup externo restaurado e autorização. `_prisma_migrations` permanece intacta como evidência histórica; futuras migrations são exclusivamente TypeORM. A tabela de controle `migrations` não é uma 24ª tabela de domínio.
...
npm ci
npm run db:local
# Somente banco vazio: confirme DATABASE_URL antes de aplicar.
...
O seed TypeORM usa códigos estáveis, transação e lock consultivo para atualizar registros conhecidos sem trocar IDs nem duplicar achievements (4), missões (3) ou cosméticos (3). A baseline cria sua estrutura; os registros são incluídos pelo seed separado.

Para testes em banco vazio/legado, geração/revisão de migrations, baseline e restauração, siga [orm-transition.md](orm-transition.md). `synchronize: true` e execução automática de migrations são proibidos. A base real não deve receber DDL da baseline.
